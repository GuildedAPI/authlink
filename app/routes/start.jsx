      ...
      <div>
        {authQuery && (
            <div className='mb-4 bg-[#3e3f4a] p-3 rounded border border-white/10 w-full'>
                ...
                <p>
                    You were about to authorize an application, but you are not signed in.
                    Complete the below process and you will be sent back to authorize the application.
                </p>
            </div>
        )}
        ...
        <div className='w-full mt-4 p-3 rounded text-lg bg-guilded-slate'>
            <label className='flex'>
                <i className={`ci-search text-2xl ${transition.state === 'submitting' ? 'animate-pulse' : ''}`}/>
                <input
        ...
        {selectedUser && (
            <div className='bg-guilded-slate w-full mt-2 rounded p-3'>
                <h2 className='font-bold text-2xl flex'>
                    <img src={selectedUser.profilePicture} className='h-12 rounded-full mr-3' alt={'Selected user\'s profile picture'}/>
                    <span className='my-auto'>{selectedUser.name}</span>
                    <span className='opacity-20 text-sm ml-auto hover:opacity-100 transition-opacity'>ID: {selectedUser.id}</span>
                </h2>
                <div className='text-center mt-4'>
                    <Button
                        ...
                        onClick={() => {
                            const authData = {}
                            if (authQueryParams) authQueryParams.forEach((val, key) => {authData[key] = val})
                            submit({id: selectedUser.id, ...authData}, {method: 'get', action: `/start/verify`})
                        }}
                    >This is me
                    </Button>
                    <button
                        ...
                        >
                            ...
                            <div className='my-auto'>
                                ...
                                {(user.aboutInfo && user.aboutInfo.tagLine) && (
                                    ...
                                    <p className='text-guilded-subtitle text-sm'>{user.aboutInfo.tagLine}</p>
                                )}
                            </div>
                            <a
        ...
        <h3 className='mt-2 text-xl font-bold text-guilded-subtitle'>Can't find yourself?</h3>
        <p className='text-guilded-subtitle'>Enter your profile URL directly to continue manually:</p>
        <div className='w-full mt-1 px-3 py-2 rounded bg-guilded-slate'>
            <label className='flex'>
                <i className={`ci-link_02 text-xl ${transition.state === 'submitting' ? 'animate-pulse' : ''}`}/>
                <input
        ...
        {loaderData.user && (
            <p className='mt-2'>
                <a href='https://www.youtube.com/watch?v=1Xfdjqa5dfY' target='_blank' className='font-bold text-rose-400 hover:underline'>Wait a minute!</a>
                {' '}
                You're already authorized as <a href={`https://www.guilded.gg/profile/${loaderData.user.id}`} target='_blank' className='font-bold hover:underline'>{loaderData.user.name}</a>. Do you really want to give that away?
            </p>
        )}
      </div>
    )
