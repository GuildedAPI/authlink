...
export async function loader({ request }) {
  ...
  try {
    const appStatement = await connection.prepare(
      "SELECT * FROM applications WHERE bot_id = $1"
    );
    appResult = await appStatement.execute({ params: [clientId] });

    const authStatement = await connection.prepare(
      "SELECT * FROM authorizations WHERE client_id = $1 AND user_id = $2"
    );
    authResult = await authStatement.execute({
      params: [clientId, guildedData.user.id],
    });
  } finally {
...
export const scopeNames = {
  ...
  "servers.members.read":
    "Your server-specific member data (nickname, roles, etc.) for public servers that you're in",
};
...
export default function Authorize() {
  ...
  return (
    <div className="w-96 max-w-full mx-auto bg-guilded-slate border border-white/10 rounded-lg p-5 h-full shadow-2xl">
      ...
      <p>
        If authorized, this application will be able to access the following
        read-only data:
      </p>
      <ul className="mt-2">
      ...
      <ul className="text-guilded-subtitle text-xs cursor-default space-y-1">
        <li className="flex">
          ...
          <p>
            This application was created on{" "}
            {createdAt.toLocaleString("en", { dateStyle: "medium" })}.
          </p>
        </li>
        {app.team_id && (
          <li className="flex">
            ...
            <p>
              You can reach {app.name}'s developer on{" "}
              <a
                href={`https://www.guilded.gg/teams/${app.team_id}`}
                target="_blank"
        ...
        <li className="flex">
          ...
          <p>
            You will be redirected to a non-Guilded site:{" "}
            <span className="text-guilded-gilded font-bold">
              {redirectUri.origin}
            </span>
          </p>
        </li>
