...
export async function loader({ request, params }) {
  ...
  try {
    ...
    result = await statement.execute({
      params: [params.bot_id, guildedData.user.id],
    });
    if (result.rows.length < 1) {
      throw json(
        {
          message:
            "You do not own an application with that ID, or it does not exist.",
        },
        { status: 404 }
      );
    }
    const authStatement = await connection.prepare(
      "SELECT COUNT(*) FROM authorizations WHERE client_id = $1"
    );
    ...
    const vanityResult = await vanityStatement.execute({
...
export async function action({ request, params }) {
  ...
  switch (data.get("_action")) {
    case "set_redirect_uris":
      ...
      try {
        const statement = await connection.prepare(
          "UPDATE applications SET redirect_uris = $1 WHERE bot_id = $2 AND owner_id = $3"
        );
    ...
    case "reset_client_secret":
      ...
      try {
        const statement = await connection.prepare(
          "UPDATE applications SET client_secret = $1 WHERE bot_id = $2 AND owner_id = $3"
        );
        await statement.execute({
          params: [newSecret, params.bot_id, guildedData.user.id],
        });
      } finally {
        await connection.close();
      }
    ...
    case "update_profile":
      ...
      try {
        const statement = await connection.prepare(
          "UPDATE applications SET name = $1, icon_hash = $2 WHERE bot_id = $3 AND owner_id = $4"
        );
        await statement.execute({
          params: [bot.name, bot.iconHash, params.bot_id, guildedData.user.id],
        });
      } finally {
        await connection.close();
      }
    ...
    case "update_linked_server":
      ...
      try {
        const statement = await connection.prepare(
          "UPDATE applications SET show_team = $1 WHERE bot_id = $2 AND owner_id = $3"
        );
        await statement.execute({
          params: [
            data.get("show_linked_server") == "on",
            params.bot_id,
            guildedData.user.id,
          ],
        });
    ...
    case "set_vanity_code":
      ...
      if (!/^[\w-]{1,100}$/.test(vanityCode)) {
        ...
        /(guilded|guiided|gullded|gulided|authlink|official|verified)/gi.test(
          vanityCode.toLowerCase()
        )
      ) {
        return {
          error: true,
          message:
            "Vanity code includes a forbidden word or phrase. If you have a legitimate use case for this phrase, please bring up this issue in guilded.gg/authlink.",
        };
      }
      ...
      try {
        ...
        const checkOwnerResult = await checkOwnerStatement.execute({
          params: [params.bot_id, guildedData.user.id],
        });
        ...
        const existStatement = await connection.prepare(
          `SELECT client_id FROM vanity_codes WHERE code = $1`
        );
        const existResult = await existStatement.execute({
          params: [vanityCode],
        });
        if (
          existResult.rows.length > 0 &&
          existResult.rows[0][0] !== params.bot_id
        ) {
          return {
            error: true,
            message:
              "This vanity code is already in use by a different application.",
          };
        }
    ...
    case "delete":
      connection = await pool.acquire();
      try {
        const appStatement = await connection.prepare(
          "DELETE FROM applications WHERE bot_id = $1 AND owner_id = $2"
        );
        const result = await appStatement.execute({
          params: [params.bot_id, guildedData.user.id],
        });
        if (result.rowsAffected > 0) {
          const authStatement = await connection.prepare(
            "DELETE FROM authorizations WHERE client_id = $1"
          );
...
export default function DevApps() {
  ...
  const [uris, setDraftingURIs] = useState(
    actionData.redirect_uris ?? app.redirect_uris
  );
    ...
    <div className="w-full">
      ...
      {showDeletePrompt && (
        <div className="mb-4 bg-[#3e3f4a] p-3 rounded border border-white/10 w-full">
          ...
          <p>
            ...
            <span className="font-bold">{app.name}</span> and its{" "}
            {app.authorization_count.toLocaleString()} active authorization
            {app.authorization_count === 1 ? "" : "s"}?
          </p>
          <div className="mt-2">
            <Button
              stylename="danger"
            ...
            <button
              className="ml-4 text-guilded-subtitle font-bold hover:text-white transition-colors"
              onClick={() => {
            ...
            items={[
              {
                ...
                callback: async () => {
                  ...
                  if (!attempt.bot) {
                    ...
                    attempt.bot.createdBy &&
                    attempt.bot.createdBy != loaderData.user.id
                  ) {
      ...
      <div className="mt-2 w-full">
        ...
        <div className="mt-4">
          ...
          <p>
            You can have up to 10 redirect URIs. Any valid URI under 2,000
        ...
        <div>
          ...
          >
            <div className="text-xl mt-2">
              <label className="flex">
                ...
                <input
                  ...
                  type="checkbox"
                  defaultChecked={
                    app.show_linked_server === null
                      ? true
                      : app.show_linked_server
                  }
            ...
            <p className="text-guilded-subtitle">
              Your bot's internal server shows up on your application's
              authorization page. If it is not an appropriate place for your
              users to contact you about your application, you can hide it by
              disabling the above tickbox.
            </p>
          </Form>
          <Form method="post" replace={true}>
            ...
            <p className="text-guilded-subtitle">
              A custom string to easily share your authorization page with your
              users, available at authlink.guildedapi.com/a/{"<code>"}. Must be
              at most 100 characters and contain only letters, numbers, hyphens,
              and underscores.
            </p>
            <div className="flex flex-wrap">
              ...
              <div className="mt-2 mr-6">
                ...
                <label>
                  <input
                    ...
                    type="checkbox"
                    value="identify"
                    defaultChecked={(loaderData.vanity.scopes ?? []).includes(
                      "identify"
                    )}
                  />{" "}
                  identify
              ...
              <div className="mt-2 mr-6">
                ...
                <label>
                  <input
                    ...
                    type="radio"
                    value="consent"
                    defaultChecked={loaderData.vanity.prompt === "consent"}
                  />{" "}
                  consent
                </label>
                <br />
                <label>
                  <input
                    ...
                    type="radio"
                    value="none"
                    defaultChecked={loaderData.vanity.prompt === "none"}
                  />{" "}
                  none
                </label>
              </div>
              <div className="mt-2 mr-6">
                ...
                >
                  ...
                  {uris.map((uri, index) => {
                    return (
                      <option key={`vanity-uri-${uri}-${index}`} value={uri}>
                        {uri}
                      </option>
                    );
                  })}
