...
export async function loader({ request }) {
  ...
  try {
    const statement = await connection.prepare(
      "SELECT * FROM applications WHERE owner_id = $1"
    );
...
export async function action({ request }) {
  ...
  if (data.get("_action") === "create") {
    const attempt = await getBotWithMethod(
      data.get("method"),
      JSON.parse(data.get("bot"))
    );
    if (attempt.error) {
      return attempt;
    }
    ...
    try {
      const getStatement = await connection.prepare(
        "SELECT name FROM applications WHERE bot_id = $1"
      );
    ...
    <div className="grow">
      ...
      {showCreatePrompt && (
        <div className="mt-2 bg-[#3e3f4a] p-3 rounded border border-white/10 w-full">
          ...
          <p className="mb-2">
            Applications are created by linking them to bots created inside the
            Guilded interface (Server settings <i className="ci-long_right" />{" "}
            Bots).
          </p>
          {strategy === "id" ? (
            ...
            <>
              ...
              {botSearchState
                .filter(
                  (botAndMeta) =>
                    botAndMeta.externalBotMeta &&
                    botAndMeta.externalBotMeta.createdBy === loaderData.user.id
                )
                .map((botAndMeta) => {
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (!team.bots) {
                          ...
                          team.bots = data.bots.filter(
                            (bot) =>
                              !!bot.enabled &&
                              !loaderData.applications
                                .map((app) => app.bot_id)
                                .includes(bot.id) &&
                              !(bot.iconUrl ?? "").includes(
                                "DefaultBotAvatars"
                              ) &&
                              !defaultBotNames.includes(bot.name) &&
                              !bot.internalBotId
                          ...
                          team.currentUser = team.members.filter(
                            (member) => member.id === loaderData.user.id
                          )[0];
                        }
                        if (
                          !team.currentUser ||
                          team.currentUser.membershipRole != "admin"
                        ) {
                          setSelectedTeam(null);
                          setErrorMsg(
                            'You must have "admin" status in this server to select a bot from it.'
                          );
                          return;
            ...
            <>
              ...
              >
                ...
                <span className="my-auto">
                  Find a server and select one of its bots
                </span>
                <i className="ci-chevron_big_right ml-auto text-xl" />
              </button>
            </>
