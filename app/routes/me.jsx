...
export async function loader({ request }) {
  ...
  try {
    const authStatement = await connection.prepare(
      "SELECT * FROM authorizations WHERE user_id = $1"
    );
    ...
    if (clientIds.length > 0) {
      const appStatement = await connection.prepare(
        "SELECT * FROM applications WHERE bot_id = ANY($1::text[])"
      );
      appResult = await appStatement.execute({
...
export async function action({ request }) {
  ...
  if (data.get("_action") === "deauthorize") {
    ...
    const connection = await pool.acquire();
    try {
      const getStatement = await connection.prepare(
        "DELETE FROM authorizations WHERE client_id = $1 AND user_id = $2"
      );
      await getStatement.execute({
        params: [data.get("id"), guildedData.user.id],
      });
    } finally {
      await connection.close();
    }
...
export const meta = () => {
  return {
    title: "Authorized Apps - Guilded Authlink",
  };
};
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="mt-2 bg-[#3e3f4a] p-3 rounded border border-white/10 w-full flex">
        <img src={profilePicture} className="rounded-full mr-3 shadow h-10" />
            ...
            items={[
              {
                ...
                label: "Your profile",
                link: {
                  to: `https://www.guilded.gg/profile/${user.id}`,
                  external: true,
                },
              },
              {
      ...
      {loaderData.applications.length === 0 ? (
        <div className="select-none text-center">
          ...
          <a
            className="text-guilded-subtitle mt-1 hover:underline"
            target="_blank"
            href="https://www.youtube.com/watch?v=NAdQyjpOfrs"
          >
            If I can't connect apps to my account, then what's this all been
            about?
          </a>
        </div>
      ) : (
        loaderData.applications.map((app) => {
          ...
          return (
            <div
              key={app.bot_id}
              className="mt-2 bg-[#3e3f4a] p-3 rounded border border-white/10 w-full group"
            >
              <div className="flex">
                <img src={iconUrl} className="rounded-full mr-3 shadow h-10" />
                <h1 className="font-bold text-lg my-auto">{app.name}</h1>
              ...
              <p className="mt-2 font-bold text-guilded-subtitle">
                This application can see:
              </p>
              <ul>
