...
export async function loader({ request, params }) {
  ...
  if (row[5] != null) {
    throw json(
      { message: "This vanity code is currently disabled." },
      { status: 404 }
    );
  }
  ...
  if (!redirectUri) {
    throw json(
      { message: `This vanity has no redirect URI connected.` + askDev },
      { status: 500 }
    );
  }
  ...
  if (!scopeStr) {
    ...
    if (!scopes) {
      throw json(
        { message: `This vanity has no scopes connected.` + askDev },
        { status: 500 }
      );
    }
  } else {
  ...
  if (prompt === "none") {
    ...
    const authResult = await authStatement.execute({
