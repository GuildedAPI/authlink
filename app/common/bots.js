    ...
    if (!bot && app.team_id) {
      ...
      if (!teamMembers.bots) {
        return {
          error: true,
          message:
            "This application's bot has been deleted or its internal server is private.",
        };
      } else {
        bot = teamMembers.bots.filter(
          (val) => (val.internalBotId || val.id) === app.bot_id
        )[0];
        if (!bot) {
          return {
            error: true,
...
export async function getBotWithMethod(method, props) {
  ...
  switch (method.toString()) {
    ...
    case "2":
      ...
      bot = (teamMembers.bots || []).filter(
        (val) => (val.internalBotId || val.id) === props.id
      )[0];
      break;
  }
  if (bot) {
    bot.iconHash = null;
    if (bot.iconUrl) {
      bot.iconHash = stripCdnUrl(bot.iconUrl);
    }
    return {
      bot,
    };
    ...
    return {
      error: true,
      message:
        "This application's bot could not be found using the method indicated.",
    };
  }
}
