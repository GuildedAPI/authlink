...
export const ErrorBlock = (props) => {
  return (
    <div
      className="rounded bg-[#df5353] border-2 border-[#e37575] px-3 py-2 mb-4 w-full font-bold empty:hidden"
      id="error"
    >
      {props.children}
    </div>
      ...
      {props.items.map((item, index) => {
        if (item.separator) {
          return (
            <hr
              key={`separator-${index}`}
              className="border-guilded-white/10 my-0.5"
            />
          );
        }
        const Wrapper = (props) => {
          return item.link ? (
            item.link.external ? (
              <a key={item.label} href={item.link.to} target="_blank">
                {props.children}
              </a>
            ) : (
              <Link key={item.label} to={item.link.to}>
                {props.children}
              </Link>
            )
          ) : (
        ...
        return (
          <Wrapper>
            ...
            >
              <div className="text-lg -mb-[2px] mt-auto mr-2 text-gray-300 group-hover:text-white transition-colors">
                {item.icon}
              </div>
              <div className="text-[0.9rem] pr-4 my-auto">{item.label}</div>
            </li>
          </Wrapper>
        );
      })}
